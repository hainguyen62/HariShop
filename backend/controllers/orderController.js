  ...
  for (const item of orderItems) {
    ...
    if (hasColors && item.color) {
      updated = await Product.findOneAndUpdate(
        {
          _id: item.product,
          colors: { $elemMatch: { name: item.color, countInStock: { $gte: item.qty } } },
        },
        { $inc: { 'colors.$.countInStock': -item.qty } },
        { session, new: true }
      )
    } else {
      updated = await Product.findOneAndUpdate(
    ...
    if (!updated) {
      const err = new Error(
        item.color
          ? `"${productName}" (màu ${item.color}) vừa hết hàng hoặc không đủ số lượng. Vui lòng quay lại giỏ hàng để cập nhật.`
          : `"${productName}" vừa hết hàng hoặc không đủ số lượng. Vui lòng quay lại giỏ hàng để cập nhật.`
      )
      err.statusCode = 409
      throw err
    }
...
const addOrderItems = asyncHandler(async (req, res) => {
  ...
  const order = new Order({
    ...
    orderItems: orderItems.map((item) => ({
      ...
      color:   item.color || '',
      // MỚI: snapshot số tháng bảo hành của sản phẩm TẠI THỜI ĐIỂM mua —
      // dùng productMap đã fetch ở trên (không tin số frontend gửi lên).
      warrantyMonths: productMap[item.product?.toString()]?.warrantyMonths ?? 12,
  ...
  try {
    ...
    if (voucherDoc) {
      ...
      if (!voucherUpdateResult) {
        const err = new Error('Mã voucher vừa hết lượt sử dụng, vui lòng quay lại giỏ hàng để cập nhật.')
        err.statusCode = 409
        throw err
      }
    }
    ...
    throw new Error(e.message || 'Không thể tạo đơn hàng, vui lòng thử lại.')
  } finally {
    session.endSession()
  }
...
// MỚI: tra cứu bảo hành công khai — không cần đăng nhập, khách vãng lai chỉ
// cần nhập mã đơn hàng + số điện thoại đặt hàng để xác minh (2 yếu tố, chống
// dò mã đơn hàng ngẫu nhiên để xem thông tin đơn của người khác).
// @route   POST /api/orders/warranty-check
// @access  Public
const checkWarranty = asyncHandler(async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(orderId)) {
    res.status(400)
    throw new Error('Mã đơn hàng không hợp lệ — vui lòng kiểm tra lại (sao chép từ trang chi tiết đơn hàng)')
  }

  const order = await Order.findById(orderId)

  // So khớp số điện thoại sau khi chuẩn hoá (bỏ khoảng trắng/dấu gạch) —
  // tránh false-negative do khách gõ "090 123 4567" thay vì "0901234567".
  ...
  const items = order.orderItems.map((item) => ({
    ...
    ...computeWarrantyStatus(item, order),
  }))
...
const cancelOrderRequest = asyncHandler(async (req, res) => {
  ...
  if (!NOT_YET_PICKED_UP.includes(currentStatus)) {
    res.status(400)
    throw new Error('Đơn hàng đã được đơn vị vận chuyển lấy hàng, không thể tự hủy. Vui lòng chờ nhận hàng và dùng chức năng yêu cầu hoàn tiền nếu cần.')
  }
  // ── MỚI: chặn gửi yêu cầu hủy trùng lặp khi đã có 1 yêu cầu đang chờ xử lý ──
  if (order.cancelRequest?.requested) {
    res.status(400); throw new Error('Bạn đã gửi yêu cầu hủy đơn này trước đó, vui lòng chờ Admin xử lý')
  }
...
const completeRefund = asyncHandler(async (req, res) => {
  ...
  // MỚI: hoàn luôn bằng totalPrice, không cho admin tự nhập số tiền
...
// @desc    Admin điều chỉnh lại số tiền hoàn cho đơn đã xác nhận trước đó
// @route   PUT /api/orders/:id/refund-adjust
// @access  Private/Admin
const adjustRefundAmount = asyncHandler(async (req, res) => {
  const { newAmount, note } = req.body

  const order = await Order.findById(req.params.id)
  if (!order) { res.status(404); throw new Error('Không tìm thấy đơn hàng') }
  if (order.refundStatus !== 'completed') {
    res.status(400); throw new Error('Chỉ có thể điều chỉnh khi đơn đã được xác nhận hoàn tiền')
  }

  const amount = Number(newAmount)
  if (!Number.isFinite(amount) || amount <= 0) {
    res.status(400); throw new Error('Số tiền điều chỉnh không hợp lệ')
  }
  if (amount === order.refundAmount) {
    res.status(400); throw new Error('Số tiền mới trùng với số tiền hiện tại, không có gì để điều chỉnh')
  }

  order.refundAdjustments.push({
    previousAmount: order.refundAmount,
    newAmount: amount,
    note: note || '',
  ...
  await createNotification({
    type: 'refund_completed',
    title: `Đơn #${order._id.toString().slice(-6).toUpperCase()} — số tiền hoàn đã được điều chỉnh`,
    message: `Số tiền hoàn mới: ${amount.toLocaleString('vi-VN')}đ`,
    link: `/order/${order._id}`,
    order: order._id,
    user: order.user,
  })
...
// @desc    Khách gửi thông tin ngân hàng để nhận lại tiền chuyển thừa
// @route   PUT /api/orders/:id/overpaid-refund-bank-info
// @access  Private (chủ đơn hàng)
const submitOverpaidRefundBankInfo = asyncHandler(async (req, res) => {
  ...
  if (order.overpaidRefundStatus !== 'pending') {
    res.status(400); throw new Error('Đơn hàng không có khoản tiền thừa đang chờ hoàn')
  }
  if (!bankName || !accountNumber || !accountHolder) {
    res.status(400); throw new Error('Vui lòng nhập đầy đủ thông tin tài khoản ngân hàng')
  }

  order.overpaidRefundBankInfo = { bankName, accountNumber, accountHolder }
  const updatedOrder = await order.save()

  await createNotification({
    type: 'payment_overpaid',
    title: `Đơn #${order._id.toString().slice(-6).toUpperCase()} — đã có thông tin ngân hàng hoàn tiền thừa`,
    message: `${order.overpaidAmount.toLocaleString('vi-VN')}đ — STK: ${accountNumber} (${bankName})`,
    link: `/order/${order._id}`,
    order: order._id,
  })
...
// MỚI: Admin xác nhận đã chuyển khoản thủ công hoàn tiền thừa (SePay không có API chuyển tiền tự động)
