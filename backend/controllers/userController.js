...
const updateUserProfile = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    res.json({
      ...
      codRestricted: updatedUser.codRestricted,
      codFailCount: updatedUser.codFailCount,
...
// ═══════════════════ Lịch sử tìm kiếm (đồng bộ đa thiết bị) ═══════════════════
...
const addSearchHistory = asyncHandler(async (req, res) => {
  ...
  user.searchHistory.unshift({ keyword, searchedAt: new Date() })
  user.searchHistory = user.searchHistory.slice(0, MAX_SEARCH_HISTORY)
