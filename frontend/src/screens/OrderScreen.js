...
const AutoCancelStatusBadge = ({ createdAt }) => {
  ...
  const color = expired || isUrgent ? '#e53935' : '#ffd166'

  return (
    <StatusBadge color={color}>
      <i className={expired ? 'fas fa-ban' : 'fas fa-hourglass-half'}></i>
      {expired ? 'Đã hủy — quá hạn thanh toán' : `Tự hủy sau ${formatCountdown(remainingMs)}`}
    </StatusBadge>
  )
}
