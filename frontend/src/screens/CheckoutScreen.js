...
const CheckoutScreen = ({ history, location }) => {
  ...
  const placeOrderHandler = () => {
    ...
    if (!(selectedAddress.phone || userInfo.phone || '').trim()) {
      alert('Địa chỉ đã chọn chưa có số điện thoại — vui lòng cập nhật lại địa chỉ, hoặc thêm SĐT trong trang Hồ sơ!')
      return
    }
    ...
    <div>
      ...
      {error && (
        <Message variant='danger'>
          {error}
          {/* MỚI (B8): nếu lỗi do giá thay đổi (vd Flash Sale hết hạn), hết hàng,
              hoặc voucher vừa hết lượt → gợi ý quay lại giỏ hàng để cập nhật */}
          {(
            (error.includes('Giá của') && error.includes('đã thay đổi')) ||
            error.includes('vừa hết hàng') ||
            error.includes('vừa hết lượt sử dụng')
          ) && (
            <>
              {' '}
              <span
      ...
      <Row className='g-4'>
        ...
        <Col lg={4}>
          <div style={{
            ...
            {/* MỚI: tính rõ TỪNG lý do khiến nút "Đặt Hàng" bị khóa, thay vì
                chỉ có 1 câu chung chung — giúp khách biết chính xác cần làm
                gì tiếp theo (thiếu địa chỉ, thiếu SĐT, hay chưa đồng ý điều
                khoản). effectivePhone khớp đúng logic thật lúc submit đơn
                (ưu tiên phone của địa chỉ, fallback qua phone tài khoản). */}
            {(() => {
              const effectivePhone = (selectedAddress?.phone || userInfo?.phone || '').trim()
              const disabledReasons = []
              if (effectiveCartItems.length === 0) {
                disabledReasons.push('Giỏ hàng đang trống')
              }
              if (!selectedAddress) {
                disabledReasons.push('Vui lòng thêm hoặc chọn địa chỉ giao hàng')
              } else if (!effectivePhone) {
                disabledReasons.push('Địa chỉ đã chọn chưa có số điện thoại — vui lòng cập nhật lại địa chỉ, hoặc thêm SĐT trong trang Hồ sơ')
              }
              if (!agreeTerms) {
                disabledReasons.push('Vui lòng đồng ý điều khoản để đặt hàng')
              }
                ...
                <>
                  {loading ? <Loader /> : (
                    <button
                      ...
                      style={{
                        width: '100%',
                        background: isDisabled ? '#444' : '#33FFCC',
                  ...
                  {disabledReasons.length > 0 && (
                    <div style={{ marginTop: '8px' }}>
                      {disabledReasons.map((reason, idx) => (
                        <p key={idx} style={{ color: '#ff6b6b', fontSize: '12px', textAlign: 'center', margin: '4px 0' }}>
                          <i className='fas fa-exclamation-circle me-1'></i>{reason}
                        </p>
                      ))}
                    </div>
                  )}
